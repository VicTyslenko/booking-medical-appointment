export class Modal {
  ...
  render() {
    ...
    // Bootstrap modal instance that controls show/hide behaviour
    this.modal = new bootstrap.Modal("#myModal", {
      keyboard: false,
    });
    this.modal.show();
    // Remove the modal element from the DOM after it finishes hiding
...
export class ModalAddCard extends Modal {
  constructor(title = "Create new visit") {
    ...
    this.body = `
        <form id="newVisitForm" class="g-3 needs-validation" novalidate>
            ...
            <!-- Shared fields (shown for all doctor types) -->
            ...
            <!-- Additional fields (doctor-specific) -->
  ...
  render() {
    ...
    // Show doctor-specific fields when the doctor selector changes
    form.addEventListener("change", (e) => {
      if (e.target !== form.querySelector("#selectDoctor")) return;
...
export class ModalEditCard extends Modal {
  ...
  render() {
    ...
    // Pre-select the current doctor type and trigger change to render its fields immediately
    const doctorSelector = form.querySelector("#selectDoctor");
